...
void main () {
    ...
    for(int i = 0; i < numLights; i++) {
        ...
        float att = attenuation( length(lights[i].position - worldPosition.xyz), 0, 1, 1, 0.2 );
        finalColor += (diffuse + (specular * vec4(lights[i].color, 1.0))) * att * lights[i].intensity;
    }

    // if(numLights > 0)
    //     finalColor /= float(numLights);

    vec4 ambient =  vec4( objectColor / 8.0f, 1.0);
