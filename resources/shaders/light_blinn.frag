...
void main () {
    ...
    if(textureEnabled == 1) {
        ...
        editedObjectColor = texColor.xyz * 1.2;
    ...
    for(int i = 0; i < numLights; i++) {
        ...
        if(lights[i].type == 0) {
            ...
            att = attenuation( length(lights[i].position - worldPosition), 1.0, 0.9, 0.32, lights[i].distance );
        }
        ...
        if(lights[i].type == 1 || lights[i].type == 2 || lights[i].type == 3) {
            ...
            spec = pow(max(dot(normalize(worldNormal), normalize(halfwayDir)), 0.0), material.shininess / 4.0);
            ...
            finalColor += (diffuse + (vec4(specular * lights[i].color * matSpecular, 1.0))) * spot * att * lights[i].intensity;
        }
